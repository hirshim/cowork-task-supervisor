...
- リストビューでタスクを作成・整理する
  ...
  - 削除・複製
  - カテゴリ分け・カテゴリフィルタ
- タスクはタイトル（任意）、プロンプトテキスト、メモ（任意）を持つ
- タスクの実行結果を保存する
...
- タスク実行前に環境準備（`prepareEnvironment`）を行う:
  1. Claude for Macの起動確認・起動（未起動時）+ バージョンチェック + UI要素検証
  2. Coworkタブ判定 → Cowork以外なら Cmd+2 で切替（マーカー存在判定）
...
- ビジー/アイドル状態の判別: AXButton label「応答を停止」の有無で判定（`isIdle()`）
- プロンプト送信:
  1. Cmd+N で新規会話を開始
  2. AXTextArea にフォーカス → クリップボード経由（Cmd+V）で入力
  3. Returnキーで送信（送信ボタンのAXPressがElectronで機能しないため）
- 応答取得:
  - 停止ボタン出現→消失を監視して完了を検知
  - テキスト抽出は4段階フォールバック（プロンプト全文→1行目→先頭80文字→最終応答抽出）
  - 後処理: 思考プロセス除去 → UIクローム除去 → 折り返し結合
- タスクキューによる順次処理（実行中に新タスクが追加された場合はキューに入り、順次実行）
- 実行中のタスクのキャンセル（応答完了後にキャンセル処理）
- 起動時に `.running`/`.queued` で残ったタスクを自動リセット（アプリ強制終了対策）
...
- 追加(+) / 複製 / 削除 | 自動実行モード切替ボタン（オフ/オン/このMac）
- アクションボタンと自動実行ボタンの間に Divider で区切り
